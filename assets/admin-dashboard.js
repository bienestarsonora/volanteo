...
function migrateState(s){
 ...
 for(const j of s.journeys||[]){
  // Las búsquedas antiguas guardaban automáticamente una ubicación como sede.
  // Desde ahora solo se conserva una referencia si fue confirmada explícitamente.
  if(j.locationReference&&j.locationReference.confirmed!==true)delete j.locationReference;
...
function setDrawingUI(active){
 ...
 const linear=mode==='perifoneo'||coverageMode==='line';
 if(btn){
  if(linear){
   if(drawing)btn.textContent=draft.length>=2?'✓ Finalizar trazado':'Trazado activo · agrega puntos';
   else btn.textContent=draft.length>=2?'✎ Continuar editando trazado':'↝ Trazar ruta lineal';
  }else btn.textContent=drawing?(draft.length>=3?'✓ Cerrar zona y calcular':'Zona activa · marca sus límites'):'▱ Delimitar zona';
 }
 if(hint){
  if(drawing)hint.textContent=coverageMode==='zone'&&mode==='volanteo'?'Marca los límites del sector. Con 3 o más puntos podrás cerrar la zona y calcular sus calles interiores.':'Haz clic sobre el mapa para marcar cada cambio de dirección. Cuando termines, pulsa “Finalizar trazado”.';
  else if(linear&&draft.length>=2)hint.textContent='Trazado finalizado. Revisa la meta y guarda la ruta, o continúa editándolo si necesitas ajustar puntos.';
...
function updateDrawingHint(){
 ...
 else{const segments=Math.max(0,draft.length-1);hint.textContent=draft.length<2?`${draft.length} punto marcado · agrega otro para formar el primer tramo.`:`${draft.length} puntos · ${segments} ${segments===1?'tramo':'tramos'} trazados · pulsa “Finalizar trazado” cuando termines.`}
 setDrawingUI(true);
}

function getJourney(){const selected=state.journeys.find(j=>j.id===state.activeJourneyId);if(selected&&!selected.archived)return selected;return state.journeys.find(j=>!j.archived)}
...
function findRouteAnywhere(id){for(const j of state?.journeys||[])for(const e of j.exercises||[])for(const r of e.routes||[])if(String(r.id)===String(id))return r;return null}
function durationShort(start,end){if(!start)return'—';const ms=Math.max(0,new Date(end||Date.now()).getTime()-new Date(start).getTime());if(!Number.isFinite(ms))return'—';const min=Math.round(ms/60000);if(min<60)return`${min} min`;return`${Math.floor(min/60)} h ${min%60} min`}
function routeEvidenceStats(r){const reports=Array.isArray(r?.blockReports)?r.blockReports:[],validated=reports.filter(x=>x.validationStatus==='validated').length,review=reports.filter(x=>x.validationStatus!=='validated').length;return{reports:reports.length,validated,review,points:Number(r?.gpsPointCount||0),distanceKm:Number(r?.gpsDistanceKm||0),duration:durationShort(r?.startedAt,r?.finishedAt),lastAt:r?.gpsLastAt||r?.lastPositionAt||null}}
function evidenceReason(rep){return rep.validationStatus==='validated'?'Evidencia GPS suficiente':(rep.validationReason||'Requiere revisión de evidencia GPS')}
function renderGpsEvidence(routeId=gpsFocusRouteId){const panel=$('#gpsEvidencePanel');if(!panel)return;const r=findRouteAnywhere(routeId);if(!r){panel.hidden=true;gpsFocusRouteId=null;return}gpsFocusRouteId=r.id;panel.hidden=false;const ev=routeEvidenceStats(r),names=routeMemberNames(r);$('#gpsEvidenceTitle').textContent=`${r.name} · recorrido observado`;$('#gpsEvidenceSubtitle').textContent=`${names.join(', ')||'Sin equipo'} · ${ev.points} puntos GPS registrados`;$('#gpsEvidenceMetrics').innerHTML=`<div><small>Distancia observada</small><strong>${ev.distanceKm.toFixed(2)} km</strong></div><div><small>Duración</small><strong>${ev.duration}</strong></div><div><small>Cuadras validadas</small><strong>${ev.validated}/${ev.reports}</strong></div><div class="${ev.review?'needs-review':''}"><small>Requieren revisión</small><strong>${ev.review}</strong></div>`;const reports=$('#gpsEvidenceReports');reports.innerHTML=(r.blockReports||[]).length?`<div class="gps-report-list-title">Validación de reportes</div>${(r.blockReports||[]).slice().reverse().map(rep=>`<article class="gps-report-item ${rep.validationStatus==='validated'?'validated':'review'}"><div><strong>Cuadra ${rep.number}</strong><small>${timeShort(rep.reportedAt)} · ${esc(rep.reportedBy||'')}</small></div><span>${rep.validationStatus==='validated'?'Validada':'Revisar'}</span><p>${esc(evidenceReason(rep))}</p><small>${Number(rep.movementM||0).toFixed(0)} m observados · ${Number(rep.gpsPoints||0)} puntos GPS${rep.accuracyM!=null?` · precisión ${Number(rep.accuracyM).toFixed(0)} m`:''}</small></article>`).join('')}`:'<div class="notice">Todavía no hay cuadras reportadas. La trayectoria GPS puede revisarse desde que inicia el recorrido.</div>';renderGpsEvidenceMap(r)}
function renderGpsEvidenceMap(r){const host=$('#gpsEvidenceMap');if(!host||host.offsetParent===null||!window.L)return;if(!gpsEvidenceMap){host.innerHTML='';gpsEvidenceMap=L.map(host,{zoomControl:true}).setView((window.VOLANTEO_CONFIG||{}).DEFAULT_CENTER||[29.0729,-110.9559],15);L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:19,attribution:'© OpenStreetMap'}).addTo(gpsEvidenceMap)}gpsEvidenceMap.eachLayer(l=>{if(!(l instanceof L.TileLayer))gpsEvidenceMap.removeLayer(l)});const bounds=[];if(r.pts?.length){if(r.type==='volanteo'&&r.coverageMode==='zone')L.polygon(r.pts,{color:r.color,weight:3,opacity:.85,fillColor:r.color,fillOpacity:.03,dashArray:'7 6'}).addTo(gpsEvidenceMap).bindTooltip('Zona programada');else L.polyline(r.pts,{color:r.color,weight:5,opacity:.72,dashArray:'9 7'}).addTo(gpsEvidenceMap).bindTooltip('Ruta programada');bounds.push(...r.pts)}for(const tr of r.gpsTracks||[]){const pts=(tr.points||[]).map(p=>[Number(p.lat),Number(p.lng)]).filter(p=>p.every(Number.isFinite));if(pts.length===1)L.circleMarker(pts[0],{radius:6,color:'#fff',weight:2,fillColor:'#177255',fillOpacity:1}).addTo(gpsEvidenceMap).bindTooltip(`${tr.name} · punto GPS`);if(pts.length>1){L.polyline(pts,{color:'#fff',weight:8,opacity:.8}).addTo(gpsEvidenceMap);L.polyline(pts,{color:'#177255',weight:5,opacity:.95}).addTo(gpsEvidenceMap).bindTooltip(`${tr.name} · recorrido GPS`)}bounds.push(...pts)}for(const rep of r.blockReports||[]){if(!Number.isFinite(Number(rep.lat))||!Number.isFinite(Number(rep.lng)))continue;const ok=rep.validationStatus==='validated';L.circleMarker([Number(rep.lat),Number(rep.lng)],{radius:7,color:'#fff',weight:3,fillColor:ok?'#239b73':'#d79221',fillOpacity:1}).addTo(gpsEvidenceMap).bindTooltip(`Cuadra ${rep.number} · ${ok?'validada':'revisar'}`);bounds.push([Number(rep.lat),Number(rep.lng)])}if(bounds.length){try{gpsEvidenceMap.fitBounds(bounds,{padding:[28,28],maxZoom:18})}catch(_e){}}setTimeout(()=>gpsEvidenceMap?.invalidateSize?.(),40)}
function openGpsEvidence(routeId){gpsFocusRouteId=routeId;renderGpsEvidence(routeId);setTimeout(()=>$('#gpsEvidencePanel')?.scrollIntoView({behavior:'smooth',block:'start'}),40)}
...
function renderJourneys(){
 ...
 if(select){
  ...
  select.innerHTML=active.length?active.map(x=>`<option value="${x.id}" ${x.id===j?.id?'selected':''}>${x.venue} · ${fmtShort(x.date)}</option>`).join(''):'<option value="">Sin jornadas activas</option>';
 }
 if(!j){
  state.activeJourneyId=null;state.activeExerciseId=null;
  $('#heroVenue').textContent='Sin jornada activa';
  $('#heroDate').textContent='Crea la próxima Jornada del Bienestar para comenzar la planeación.';
  ['#editJourneyBtn','#archiveJourneyBtn','#deleteJourneyBtn'].forEach(id=>{const b=$(id);if(b)b.disabled=true});
  const timeline=$('#exerciseTimeline');if(timeline)timeline.innerHTML='<div class="notice">No hay una jornada activa. Usa “+ Nueva jornada” para registrar la siguiente.</div>';
  const tracking=$('#trackingList');if(tracking)tracking.innerHTML='<div class="notice">El seguimiento aparecerá cuando existan rutas.</div>';
  const routes=$('#journeyRouteList');if(routes)routes.innerHTML='<div class="notice">Sin cobertura registrada.</div>';
  ['#mExercises','#mRoutes','#mPeople'].forEach(id=>{const el=$(id);if(el)el.textContent='0'});if($('#mCoverage'))$('#mCoverage').textContent='0%';
  renderHistory();renderRosterSummary();
  if(map)drawMap();
  return;
 }
 state.activeJourneyId=j.id;if(!j.exercises?.some(e=>e.id===state.activeExerciseId))state.activeExerciseId=j.exercises?.[0]?.id||null;
 $('#heroVenue').textContent=j.venue;$('#heroDate').textContent=`${fmtDate(j.date)} · ${j.time} h`;
 ['#editJourneyBtn','#archiveJourneyBtn','#deleteJourneyBtn'].forEach(id=>{const b=$(id);if(b)b.disabled=false});
...
function renderTracking(){const e=getExercise(), el=$('#trackingList');const routes=e?.routes||[];el.innerHTML=routes.length?routes.map(r=>routeCard(r,true)).join(''):'<div class="notice">Este ejercicio todavía no tiene rutas. Diseña la primera desde el panel de planeación.</div>';bindRouteActions(el);if(gpsFocusRouteId&&routes.some(r=>String(r.id)===String(gpsFocusRouteId)))renderGpsEvidence(gpsFocusRouteId);else if(gpsFocusRouteId){gpsFocusRouteId=null;const p=$('#gpsEvidencePanel');if(p)p.hidden=true}}
...
function routeCard(r,actions){const status=r.status==='done'?'Finalizada':r.status==='live'?'En recorrido':'Pendiente';const cls=r.status==='done'?'done':r.status==='live'?'live':'';const names=routeMemberNames(r),op=routeOperationalStats(r),ev=routeEvidenceStats(r);const goalText=r.type==='perifoneo'?`${Number(op.goal).toFixed(1)} km`:`${op.goal} ${op.unit}`;const reportText=r.coverageMode==='zone'&&r.type!=='perifoneo'?`<div class="live-route-meta"><span><strong>${op.done}/${op.goal}</strong> cuadras reportadas</span><span class="gps-valid-count">GPS: <strong>${ev.validated}</strong> validadas${ev.review?` · <b>${ev.review} revisar</b>`:''}</span>${r.lastPositionAt?`<span class="live-gps-dot">Último GPS ${timeShort(r.lastPositionAt)}</span>`:''}</div>`:'';const gpsSummary=`<div class="gps-route-summary ${ev.review?'has-review':''}"><span><strong>${ev.distanceKm.toFixed(2)} km</strong> observados</span><span><strong>${ev.points}</strong> puntos GPS</span><span><strong>${ev.duration}</strong> en campo</span></div>`;return `<article class="tracking-card" style="--route-color:${r.color}"><div class="tracking-head"><div><div class="tracking-title"><span class="swatch"></span><strong>${r.name}</strong><span class="pill">${r.type==='perifoneo'?'Perifoneo':'Volanteo'}</span></div><small>${names.length} ${names.length===1?'brigadista':'brigadistas'} · ${goalText}</small></div><span class="badge ${cls}">${status}</span></div><div class="member-chips">${names.map(n=>`<span class="member-chip">${n}</span>`).join('')}</div>${reportText}${gpsSummary}<div class="progress"><span style="width:${r.progress||0}%"></span></div><div style="display:flex;justify-content:space-between;margin-top:6px"><small>Avance reportado</small><strong>${r.progress||0}%</strong></div>${actions?`<div class="card-actions">${ev.points?`<button class="btn btn-primary btn-sm" data-gpsroute="${r.id}">Ver recorrido GPS</button>`:''}<button class="btn btn-secondary btn-sm" data-edit="${r.id}">Editar</button><button class="btn btn-secondary btn-sm" data-duplicate="${r.id}">Duplicar</button><button class="btn btn-danger btn-sm" data-delete="${r.id}">Borrar</button></div>`:''}</article>`}
function bindRouteActions(scope=document){scope.querySelectorAll('[data-gpsroute]').forEach(b=>b.addEventListener('click',()=>openGpsEvidence(b.dataset.gpsroute)));scope.querySelectorAll('[data-edit]').forEach(b=>b.addEventListener('click',()=>openEditRoute(b.dataset.edit)));scope.querySelectorAll('[data-duplicate]').forEach(b=>b.addEventListener('click',()=>duplicateRoute(b.dataset.duplicate)));scope.querySelectorAll('[data-delete]').forEach(b=>b.addEventListener('click',()=>deleteRoute(b.dataset.delete)))}

function renderHistory(){const el=$('#historyList');if(!el)return;const archived=state.journeys.filter(j=>j.archived);el.innerHTML=archived.length?archived.map(j=>{const rs=allRoutes(j),avg=rs.length?Math.round(rs.reduce((a,r)=>a+(r.progress||0),0)/rs.length):0;return `<article class="history-card" data-history="${j.id}"><div class="history-top"><div><strong>Jornada del Bienestar · ${j.venue}</strong><small>${fmtDate(j.date)} · ${j.time} h</small></div><span class="pill">Archivada</span></div><div class="history-stats"><div><strong>${j.exercises.length}</strong><small>ejercicios</small></div><div><strong>${rs.length}</strong><small>rutas</small></div><div><strong>${avg}%</strong><small>cobertura</small></div></div><div class="card-actions"><button class="btn btn-secondary btn-sm" data-editjourney="${j.id}">Editar</button><button class="btn btn-secondary btn-sm" data-restore="${j.id}">Restaurar</button><button class="btn btn-danger btn-sm" data-deletejourney="${j.id}">Eliminar</button></div></article>`}).join(''):'<div class="notice">Las jornadas que archives aparecerán aquí para conservar su memoria operativa.</div>';
 el.querySelectorAll('[data-editjourney]').forEach(b=>b.addEventListener('click',e=>{e.stopPropagation();openEditJourney(b.dataset.editjourney)}));
 el.querySelectorAll('[data-restore]').forEach(b=>b.addEventListener('click',async e=>{e.stopPropagation();const j=state.journeys.find(x=>x.id===b.dataset.restore);if(!j)return;j.archived=false;state.activeJourneyId=j.id;state.activeExerciseId=j.exercises[0]?.id||null;await saveState(state,{immediate:true});renderJourneys()}));
 el.querySelectorAll('[data-deletejourney]').forEach(b=>b.addEventListener('click',e=>{e.stopPropagation();deleteJourney(b.dataset.deletejourney)}));
}
...
function addRouteToMap(r,opacity=.82,{showStreetSegments=true}={}){
 ...
 if(r.type==='volanteo'&&r.coverageMode==='zone'){
  ...
  (r.blockReports||[]).filter(x=>Number.isFinite(Number(x.lat))&&Number.isFinite(Number(x.lng))).forEach(rep=>{const ok=rep.validationStatus==='validated';L.circleMarker([Number(rep.lat),Number(rep.lng)],{radius:5,color:'#fff',weight:2,fillColor:ok?'#239b73':'#d79221',fillOpacity:1}).addTo(map).bindTooltip(`Cuadra ${rep.number} · ${ok?'validada por GPS':'requiere revisión'}`)})
 }else L.polyline(r.pts,{color:r.color,weight:6,opacity}).addTo(map).bindTooltip(`${r.name} · ${routeMemberNames(r).join(', ')||'Sin equipo'}`);
 for(const tr of r.gpsTracks||[]){const pts=(tr.points||[]).map(p=>[Number(p.lat),Number(p.lng)]).filter(p=>p.every(Number.isFinite));if(pts.length>1){L.polyline(pts,{color:'#fff',weight:7,opacity:.7}).addTo(map);L.polyline(pts,{color:'#177255',weight:4,opacity:.9}).addTo(map).bindTooltip(`${r.name} · recorrido GPS · ${tr.name}`)}}
...
function clearMapSearchPreview({clearInput=false}={}){
 ...
 if(searchPreviewMarker&&map&&map.hasLayer(searchPreviewMarker))map.removeLayer(searchPreviewMarker);
 searchPreviewMarker=null;
 const results=$('#mapSearchResults');if(results){results.hidden=true;results.innerHTML=''}
 if(clearInput&&$('#mapSearchInput'))$('#mapSearchInput').value='';
 const status=$('#mapSearchStatus');if(status)status.textContent='El buscador solo te orienta: no fija ni guarda la sede de la Jornada.';
}
function drawMapSearchPreview(){
 searchPreviewMarker=null;if(!map||!mapSearchPreview)return null;
 const lat=Number(mapSearchPreview.lat),lng=Number(mapSearchPreview.lng);if(!Number.isFinite(lat)||!Number.isFinite(lng))return null;
 searchPreviewMarker=L.circleMarker([lat,lng],{radius:8,color:'#fff',weight:3,fillColor:'#5f6670',fillOpacity:.92,dashArray:'3 2'}).addTo(map)
  .bindTooltip('Referencia de búsqueda',{direction:'top'}).bindPopup(`<strong>Referencia temporal</strong><br>${esc(mapSearchPreview.label||'Ubicación encontrada')}<br><small>No se guarda como sede.</small>`);
 return searchPreviewMarker;
}
async function searchMapLocation(ev){
 ...
 const q=normalizeName(input?.value);if(!q){clearMapSearchPreview({clearInput:true});return}
 if(results){results.hidden=true;results.innerHTML=''}
 if(status)status.textContent='Buscando referencia…';if(btn)btn.disabled=true;
 try{
  ...
  if(!Array.isArray(data)||!data.length){if(status)status.textContent='No encontramos esa referencia. Prueba con colonia + Hermosillo o una dirección más específica.';return}
  if(results){
   results.innerHTML=data.map((r,i)=>`<button type="button" class="map-search-result" data-map-result="${i}"><strong>${esc(String(r.display_name||'').split(',').slice(0,2).join(','))}</strong><small>${esc(r.display_name||'')}</small></button>`).join('');
   results.hidden=false;
   results.querySelectorAll('[data-map-result]').forEach(b=>b.addEventListener('click',()=>{
    const r=data[Number(b.dataset.mapResult)],lat=Number(r.lat),lng=Number(r.lon);if(!Number.isFinite(lat)||!Number.isFinite(lng))return;
    mapSearchPreview={lat,lng,label:r.display_name||q};
    results.hidden=true;
    if(input)input.value=String(r.display_name||q).split(',').slice(0,3).join(',');
    if(status)status.textContent='Referencia temporal. Puedes moverte libremente por el mapa; esta ubicación NO se guarda como sede.';
    // Redibujar capas sin modificar el encuadre actual y luego ir una sola vez al resultado.
    drawMap({preserveView:true});
...
function drawVenueReference(j){
 venueMarker=null;if(!map||!j?.locationReference||j.locationReference.confirmed!==true)return null;
 const lat=Number(j.locationReference.lat),lng=Number(j.locationReference.lng);if(!Number.isFinite(lat)||!Number.isFinite(lng))return null;
 venueMarker=L.circleMarker([lat,lng],{radius:9,color:'#fff',weight:3,fillColor:'#c41467',fillOpacity:1}).addTo(map).bindTooltip('Sede confirmada de la Jornada',{direction:'top',className:'venue-map-tooltip'}).bindPopup(`<strong>Sede confirmada</strong><br>${esc(j.locationReference.label||j.venue)}`);return venueMarker;
}
function initMap(){
 if(!window.L){$('#adminMap').innerHTML='<div class="map-fallback">El mapa no pudo cargar. La administración y los formularios siguen disponibles.</div>';return}
 map=L.map('adminMap').setView((window.VOLANTEO_CONFIG||{}).DEFAULT_CENTER||[29.0729,-110.9559],14);
 L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:19,attribution:'© OpenStreetMap'}).addTo(map);
 map.on('click',e=>{if(!drawing)return;const point=[Number(e.latlng.lat),Number(e.latlng.lng)];draft=[...draft,point];clearRoadAnalysis();refreshDraft();updateGoal();updateDrawingHint()});
 drawMap({forceFit:true});
 /* El paneo y zoom del usuario nunca se revierten por la sincronización periódica. */
}
function drawMap({forceFit=false,preserveView=false}={}){
 if(!map)return;
 const previousCenter=map.getCenter(),previousZoom=map.getZoom();
 map.eachLayer(l=>{if(!(l instanceof L.TileLayer))map.removeLayer(l)});
 draftLayer=null;draftVertexLayer=null;roadLayer=null;venueMarker=null;searchPreviewMarker=null;
 const j=getJourney(),e=getExercise();
 if(!j){drawMapSearchPreview();return}
 drawVenueReference(j);
 const contextKey=`${j.id}:${e?.id||'none'}:${mapFilter}`;
 ...
 if(e){
  ...
  if(!preserveView&&(forceFit||contextChanged)){
   ...
   if(all.length)map.fitBounds(all,{padding:[35,35],maxZoom:16});
   else if(j.locationReference?.confirmed===true)map.setView([Number(j.locationReference.lat),Number(j.locationReference.lng)],16);
   else if(forceFit)map.setView((window.VOLANTEO_CONFIG||{}).DEFAULT_CENTER||[29.0729,-110.9559],14);
  }else if(preserveView&&previousCenter)map.setView(previousCenter,previousZoom,{animate:false});
 }else{
  if(!preserveView&&(forceFit||contextChanged)){
   if(j.locationReference?.confirmed===true)map.setView([Number(j.locationReference.lat),Number(j.locationReference.lng)],16);
   else if(forceFit)map.setView((window.VOLANTEO_CONFIG||{}).DEFAULT_CENTER||[29.0729,-110.9559],14);
  }else if(preserveView&&previousCenter)map.setView(previousCenter,previousZoom,{animate:false});
 }
...
function bind(){
 renderMemberPicker('#memberPicker');
 $('#closeGpsEvidenceBtn')?.addEventListener('click',()=>{gpsFocusRouteId=null;const p=$('#gpsEvidencePanel');if(p)p.hidden=true});
 $('#mapSearchForm')?.addEventListener('submit',searchMapLocation);
 $('#mapSearchInput')?.addEventListener('input',e=>{if(!String(e.target.value||'').trim())clearMapSearchPreview()});
 $('#mapSearchInput')?.addEventListener('keydown',e=>{if(e.key==='Escape'){e.preventDefault();clearMapSearchPreview({clearInput:true})}});
 $('#journeySelect').addEventListener('change',e=>{state.activeJourneyId=e.target.value||null;state.activeExerciseId=getJourney()?.exercises?.[0]?.id||null;mapSearchPreview=null;mapContextKey=null;saveState();renderJourneys()});
 ...
 $('#startDrawBtn').addEventListener('click',async()=>{
  const linear=mode==='perifoneo'||coverageMode==='line';
  if(drawing&&linear){
   if(draft.length<2)return alert('Marca al menos dos puntos para formar un tramo antes de finalizar.');
   setDrawingUI(false);refreshDraft();updateGoal();
   const summary=mode==='perifoneo'?`${draftKm().toFixed(1)} km`:`${Math.max(0,draft.length-1)} ${draft.length-1===1?'tramo':'tramos'}`;
   $('#drawNotice').className='notice success';
   $('#drawNotice').textContent=`Trazado finalizado · ${summary}. Ya puedes guardar la ruta o continuar editándolo si necesitas ajustar algún punto.`;
   return;
  }
  if(drawing&&mode==='volanteo'&&coverageMode==='zone'&&draft.length>=3){setDrawingUI(false);await analyzeZone(draft);$('#drawNotice').className=roadAnalysis?'notice success':'notice';$('#drawNotice').textContent=roadAnalysis?`Zona analizada: ${roadAnalysis.streetCount} calles operativas y ${roadAnalysis.blockCount} cuadras estimadas.`:'No pudimos analizar las calles todavía. Puedes reintentar sin volver a dibujar.';return}
  if(!drawing){
   const resumeLinear=linear&&draft.length>=2;
   drawSessionId++;analysisRunId++;
   if(!resumeLinear){draft=[];clearRoadAnalysis()}
   setDrawingUI(true);refreshDraft();updateGoal();updateDrawingHint();$('#drawNotice').className='notice success';
   $('#drawNotice').textContent=resumeLinear?'Edición reactivada. Puedes agregar más puntos y volver a finalizar cuando termines.':(coverageMode==='zone'&&mode==='volanteo'?'Delimitación activa. Marca los límites del sector y después cierra la zona.':'Trazado activo. El mapa está listo para recibir puntos.');
   if(window.innerWidth<1020)$('#adminMap')?.scrollIntoView({behavior:'smooth',block:'center'});
  }
 });
 $('#undoBtn').addEventListener('click',()=>{if(!drawing&&(mode==='perifoneo'||coverageMode==='line')&&draft.length)setDrawingUI(true);draft=draft.slice(0,-1);analysisRunId++;clearRoadAnalysis();refreshDraft();updateGoal();updateDrawingHint()});$('#clearBtn').addEventListener('click',()=>{drawSessionId++;analysisRunId++;draft=[];clearRoadAnalysis();setDrawingUI(false);refreshDraft();updateGoal();$('#drawNotice').className='notice';$('#drawNotice').textContent='Trazado reiniciado. Puedes comenzar de nuevo cuando quieras.'});$('#saveRouteBtn').addEventListener('click',saveRoute);$('#saveEditRouteBtn').addEventListener('click',saveEditRoute);

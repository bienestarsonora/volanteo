...
async function saveRoute(){
 ...
 try{
  ...
  renderJourneyRoutes();renderTracking();renderSideRoutes?.();
  return;
 }
...
async function submitAdminLogin(event){
 ...
 if(!email||!password){if(n){n.hidden=false;n.className='notice';n.textContent='Captura correo y contraseña.'}return}
 if(!window.VolanteoCloud?.enabled){if(n){n.hidden=false;n.className='notice';n.textContent='La conexión con Supabase no está configurada.'}return}
 if(n){n.hidden=false;n.className='notice';n.textContent='Validando acceso…'}
 ...
 try{
  ...
  if(n)n.textContent='Cuenta verificada. Comprobando permisos…';
  const allowed=await window.VolanteoCloud.isAdmin();
  if(!allowed){await window.VolanteoCloud.adminSignOut();throw new Error('Esta cuenta no tiene permisos de administrador.')}
  if(n){n.className='notice success-note';n.textContent='Acceso correcto. Cargando panel…'}
  // No recargamos: evita caché y hace visible cualquier fallo del arranque.
  await bootAdmin();
 }catch(err){
  console.error('Error de acceso administrativo',err);
  if(n){n.hidden=false;n.className='notice';n.textContent=err?.message||'No fue posible iniciar sesión.'}
 }finally{
